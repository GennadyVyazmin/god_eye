...
class NearestNeighborDistanceMetric:
    ...
    def _cosine_distance(self, x, y):
        """
        ИСПРАВЛЕННАЯ версия косинусного расстояния
        """
        x = np.asarray(x, dtype=np.float32)
        y = np.asarray(y, dtype=np.float32)

        # Если x и y - векторы (1D), преобразуем их в 2D
        if x.ndim == 1:
            x = x.reshape(1, -1)
        if y.ndim == 1:
            y = y.reshape(1, -1)
    ...
    def distance(self, features, targets):
        """
        ИСПРАВЛЕННАЯ версия вычисления расстояния
        """
        ...
        for i, feature in enumerate(features):
            for j, target in enumerate(targets):
                if target in self.samples and len(self.samples[target]) > 0:
                    ...
                    cost_matrix[i, j] = self._metric([feature], [target_feature])[0, 0]
                else:
